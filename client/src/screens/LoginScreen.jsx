  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (response) => {
      try {
        ...
        const backendResponse = await axios.post('http://localhost:3000/api/users/google-login', {
          googleId,
          email,
          name,
          picture,
        ...
        // Process the backend response (store token, user info, etc.)
        dispatch({ type: 'LOGIN_SUCCESS', payload: backendResponse.data });
        navigate('/');  // Redirect the user to the homepage or another page
      } catch (error) {
